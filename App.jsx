...
async function generatePDF(config) {
  ...
  const navy = [27, 42, 74];
  const medBlue = [58, 107, 197];
  const gray = [107, 123, 149];
  const lightBlueBg = [244, 246, 250];
  const lightGray = [208, 215, 227];
  const green = [39, 174, 96];
  const gold = [240, 192, 64];
  const white = [255, 255, 255];
  const darkText = [74, 91, 117];
  const dimmedStrike = [160, 170, 185];
  const featuredStrike = [100, 120, 160];

  const headerBottom = H - 65;
  const footerTop = 55;
  ...
  function drawLogoDots(x, baseY, size, lightCol, darkCol) {
    const r = size;
    const gap = size * 2.8;
    setF(lightCol);
    doc.circle(x, baseY - gap, r, "F");
    doc.circle(x + gap, baseY - gap, r, "F");
    setF(darkCol || medBlue);
    doc.circle(x, baseY, r, "F");
    doc.circle(x + gap, baseY, r, "F");
    doc.circle(x + gap * 0.5, baseY + gap, r, "F");
    doc.circle(x + gap * 1.5, baseY + gap, r, "F");
  }
  ...
  function drawHeader(pageNum) {
    ...
    const ew = doc.getTextWidth("ENUMERATE");
    drawLogoDots(M + ew + 7, 28, 2, [130, 180, 230], medBlue);

    doc.setFontSize(8);
    doc.setFont("helvetica", "normal");
    setC(gray);
    doc.text("Page " + pageNum, W - M, 30, { align: "right" });
  ...
  doc.rect(0, 0, W, H, "F");
  setF([31, 48, 80]);
  doc.triangle(W * 0.55, 0, W, 0, W, H * 0.7, "F");
  ...
  for (let row = 0; row < 20; row++) {
    for (let col = 0; col < 15; col++) {
      if ((row + col) % 3 === 0) {
        setF([42, 63, 101]);
        doc.circle(40 + col * 40, 40 + row * 40, 1.5, "F");
      }
    }
  }
  ...
  doc.setFont("helvetica", "bold");
  setC(white);
  doc.text("ENUMERATE", 60, 85);
  const ew1 = doc.getTextWidth("ENUMERATE");
  drawLogoDots(60 + ew1 + 9, 82, 3.5, [130, 180, 230], medBlue);

  setD(medBlue);
  doc.setLineWidth(2);
  doc.line(60, 105, 200, 105);
